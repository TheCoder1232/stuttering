...
import { Mic, Square, RotateCcw, Activity, FileText, ChevronRight, AlertCircle, CheckCircle, Loader2 } from 'lucide-react';
      ...
      <div className="w-full">
        ...
        <header className="mb-8 flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-500">AI-Powered Speech Therapy Assistant</p>
          </div>
          <div className="bg-white px-4 py-2 rounded-full shadow-sm border border-slate-200">
            <span className="text-sm font-semibold text-slate-600">User: Final Year Student</span>
          </div>
        </header>
        ...
        {error && (
          <div className="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-md flex items-start gap-3">
            ...
            <div>
              <h3 className="text-red-800 font-bold text-sm">Analysis Failed</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100 relative overflow-hidden">
              ...
              <div className="mt-6 flex gap-2">
                <button 
                  onClick={() => setCurrentPrompt((prev) => (prev + 1) % prompts.length)}
            ...
            <div className="bg-white rounded-2xl p-6 shadow-md border border-slate-100 flex flex-col items-center justify-center min-h-[250px]">
              ...
              {!recording && !analyzing && !results && (
                <div className="text-center">
                  ...
                  <p className="mt-4 text-slate-600 font-medium">Tap to Start Recording</p>
                </div>
              )}
              ...
              {recording && (
                <div className="text-center animate-pulse">
                  ...
                  <p className="mt-4 text-red-500 font-medium">Recording... (Tap to Stop)</p>
                  <div className="mt-2 flex gap-1 justify-center h-4 items-end">
                    {[...Array(5)].map((_, i) => (
                      <div key={i} className="w-1 bg-red-400 rounded-full animate-bounce" style={{height: Math.random() * 20 + 10 + 'px', animationDelay: i * 0.1 + 's'}}></div>
                    ))}
                  </div>
                </div>
              )}
              ...
              {analyzing && (
                <div className="text-center">
                  ...
                  <p className="mt-4 text-indigo-600 font-medium">Analyzing Speech Patterns...</p>
                  <p className="text-xs text-slate-400 mt-1">Processing audio on local GPU...</p>
                </div>
              )}
              ...
              {results && !analyzing && (
                <div className="w-full animate-in fade-in duration-500">
                  <div className="flex justify-between items-center mb-4 border-b pb-2">
                    ...
                    <button onClick={() => setResults(null)} className="text-sm text-slate-500 hover:text-indigo-600 flex items-center gap-1">
                  ...
                  <div className="bg-slate-50 rounded-lg p-4 border border-slate-200 mb-4">
                    <h4 className="text-xs font-bold text-slate-400 uppercase mb-2">Transcript & Stutters</h4>
                    <p className="text-lg text-slate-700 leading-relaxed">
                      {results.original_transcript ? (
                         results.original_transcript
                      ) : (
                         <span className="italic text-slate-400">Audio processed, but transcription failed.</span>
                      )}
                    </p>
                    {/* Render Stutter Tags */}
                    {results.events.length > 0 ? (
                      <div className="mt-4 flex flex-wrap gap-2">
                        {results.events.map((event, idx) => (
                          <span key={idx} className="px-2 py-1 text-xs font-bold rounded bg-red-100 text-red-700 border border-red-200">
                      ...
                      <div className="mt-4 flex items-center gap-2 text-green-600 text-sm font-medium">
                        <CheckCircle className="w-4 h-4" /> No stutters detected.
                      </div>
                    )}
                  </div>
                  ...
                  {/* Correction */}
                   <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-100">
                    <h4 className="text-xs font-bold text-indigo-300 uppercase mb-1">Corrected Text</h4>
                    <p className="text-indigo-800">{results.corrected_transcript}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-md border border-slate-100">
              <h3 className="text-slate-500 font-bold text-xs uppercase tracking-wider mb-2">Session Fluency Score</h3>
              <div className="flex items-end gap-2">
                <span className={`text-6xl font-bold ${results ? (results.fluency_score > 80 ? 'text-green-500' : 'text-yellow-500') : 'text-slate-200'}`}>
                  {results ? results.fluency_score : '--'}
                </span>
                <span className="text-xl text-slate-400 mb-2">/ 100</span>
              </div>
              <p className="text-xs text-slate-400 mt-2">
                {results ? "Score calculated based on duration and frequency of dysfluencies." : "Complete a recording to see your score."}
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-md border border-slate-100 h-full">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-bold text-slate-700 text-sm mb-1">Therapist Feedback</h4>
                  <ul className="text-sm text-slate-600 space-y-2 mt-2">
                    {results ? (
                      ...
                      <li className="italic text-slate-400">Feedback will appear here after analysis.</li>
                    )}
                  </ul>
                </div>
              </div>
